...
(function () {
  ...
  function drawMap(run) {
    ...
    run.nodes.forEach(function (n) {
      ...
      // El tipo de destino se conoce por su firma de red; escanear sigue siendo
      // necesario para revelar nombre, firewall y botín. Así elegir una rama es
      // una decisión informada en vez de escoger entre tres cajas idénticas.
      ctx.fillText(KIND_GLYPH[n.kind] || '?', x + 16, y + 20);
      // nombre
      ...
      if (visible && !n.drained && n.kind !== 'loot' && n.kind !== 'shop' && n.kind !== 'event' && n.kind !== 'isp') {
        ...
        ctx.fillText('FW ' + n.fw + ' · ' + (NS.I18n && NS.I18n.get() === 'en' ? 'Lv ' : 'Nv ') + n.lvl, x + w / 2, y + 15);
      } else if (visible && n.kind === 'loot') {
        ctx.fillStyle = '#8fc89a';
        ctx.fillText(NS.I18n ? NS.I18n.t('botín') : 'botín', x + w / 2, y + 15);
      } else if (visible && n.kind === 'shop') {
        ctx.fillStyle = '#d6c06a';
        ctx.fillText(NS.I18n ? NS.I18n.t('mercado') : 'mercado', x + w / 2, y + 15);
      } else if (visible && n.kind === 'event') {
        ctx.fillStyle = '#b99ad6';
        ctx.fillText(NS.I18n ? NS.I18n.t('evento') : 'evento', x + w / 2, y + 15);
    ...
    ctx.font = '9px Tahoma';
    ctx.fillText(NS.I18n ? NS.I18n.t('Haz clic en un nodo alcanzable para atacarlo · Drena nodos conectados para abrir el camino') : 'Haz clic en un nodo alcanzable para atacarlo · Drena nodos conectados para abrir el camino', 12, H - 6);
  }
  ...
  function render(body) {
    ...
    // consola
    var consWrap = Util.el('details', { class: 'net-console-wrap' });
    var consTitle = Util.el('summary', { text: 'Consola avanzada' });
    ...
    consWrap.appendChild(consTitle);
    consWrap.appendChild(cons);
    body.appendChild(consWrap);
